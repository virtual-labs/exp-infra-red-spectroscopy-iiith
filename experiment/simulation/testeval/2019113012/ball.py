...
class gameball:
    ...
    def getpaddleflag(self):
        return self.paddlecollidflag
    def setpaddleflag(self,var):
        self.paddlecollidflag = var
...
class multiballs:
    ...
    def checkbrickcollision(self,brick,pwrup,tdarr,scr):
        ...
        for i in range(len(self.ballarr)):
            ...
            if iden != -1:
                tdarr,brickdestroy = brick.removebrick(tdarr,iden)
                pwrup.createpowerup(tdarr,brick.getarray(),iden,brick.retrunidval(),self.ballarr[i].getxvel(),self.ballarr[i].getyvel())
                scr+=1
            self.ballarr[i].updateveloc(x,y)
        return brick,pwrup,tdarr,scr,brickdestroy
